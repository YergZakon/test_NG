...
MEDIUM_RISK_QUESTIONS = {
    ...
    "isolation": [
        ...
        {"id": "is_med6", "text": "Я чувствую себя совершенно одиноким."}
    ],
    "somatic": [
        {"id": "som_med1", "text": "Иногда меня охватывает чувство ужаса"},
...
MILITARY_QUESTIONNAIRE = {
    ...
    "health_history": {
        ...
        "questions": [
            ...
            {"id": "personal_psych", "text": "Были ли у вас до армии нервно-психические заболевания", "type": "radio", "options": ["Да", "Нет"], "required": True},
    ...
    "religion_lifestyle": {
        ...
        "questions": [
            {"id": "religion", "text": "Какую религию исповедуешь", "type": "text", "required": False},
            {"id": "religion_path", "text": "Какое направление религии", "type": "text", "required": False},
            {"id": "religion_teacher", "text": "Если ты слушаешь духовных учителей, то перечисли их", "type": "text", "required": False},
...
def analyze_results():
    ...
    for scale, score in scores.items():
        if scale == 'sincerity':
            ...
            if score <= 5:  # Слишком низкие баллы по искренности
                risk_levels[scale] = "low_sincerity"
                risk_levels_desc[scale] = "низкий уровень искренности (возможна социальная желательность)"
                # Переход к предупреждению о недостоверности
                st.session_state.stage = 'sincerity_warning'
                return
            else:
                risk_levels[scale] = "normal"
                risk_levels_desc[scale] = "нормальный уровень искренности"
            continue
        
        # Определение уровня риска для остальных шкал
        if score <= THRESHOLDS["low"][1]:
            ...
            risk_levels_desc[scale] = "средний уровень риска"
            medium_risk_scales.append(scale)
        else:
            risk_levels[scale] = "high"
            risk_levels_desc[scale] = "высокий уровень риска"
            high_risk_scales.append(scale)
    ...
    if high_risk_scales:
        # Есть шкалы с высоким риском - углубленная оценка
...
def prepare_final_recommendations():
    """Подготовка финальных рекомендаций"""
    recommendations = []
    ...
    for scale, level in st.session_state.risk_levels_desc.items():
        ...
        if "высокий" in level:
            if scale == "aggression":
                recommendations.append("🔴 [КРИТИЧНО] Требуется работа с психологом по управлению агрессией")
            elif scale == "military_adaptation":
                recommendations.append("🔴 [КРИТИЧНО] Необходима специальная программа военной адаптации")
            elif scale == "stability":
                recommendations.append("🔴 [КРИТИЧНО] Требуется укрепление нервно-психической устойчивости")
            elif scale == "anxiety":
                recommendations.append("🔴 [КРИТИЧНО] Необходима работа по снижению тревожности")
            elif scale == "isolation":
                recommendations.append("🔴 [КРИТИЧНО] Требуется развитие социальных навыков")
            elif scale == "somatic":
                recommendations.append("🔴 [КРИТИЧНО] Необходима работа с психосоматическими проявлениями")
        
        elif "средний" in level:
            if scale == "aggression":
                recommendations.append("🟡 [ВНИМАНИЕ] Рекомендуются техники самоконтроля")
            elif scale == "military_adaptation":
                recommendations.append("🟡 [ВНИМАНИЕ] Дополнительная подготовка к военной службе")
            elif scale == "stability":
                recommendations.append("🟡 [ВНИМАНИЕ] Тренинги стрессоустойчивости")
            elif scale == "anxiety":
                recommendations.append("🟡 [ВНИМАНИЕ] Методы релаксации и управления тревогой")
            elif scale == "isolation":
                recommendations.append("🟡 [ВНИМАНИЕ] Развитие навыков командной работы")
            elif scale == "somatic":
                recommendations.append("🟡 [ВНИМАНИЕ] Профилактика стрессовых соматических проявлений")
    
    # Общие рекомендации
    recommendations.append("✅ [ОБЩЕЕ] Регулярные занятия спортом для поддержания физической формы")
    recommendations.append("✅ [ОБЩЕЕ] Соблюдение режима дня и полноценный сон")
    recommendations.append("✅ [ОБЩЕЕ] Поддержание связи с семьей и друзьями")
...
def complete_detailed_assessment(risk_level):
    ...
    if question_count > 0:
        ...
        if percentage >= 70:
            ...
            st.session_state.risk_levels_desc[current_scale] = "низкий уровень риска (скорректировано после углубленной оценки)"
    ...
    if risk_level == "high":
        ...
        if remaining_high_risk:
            ...
            # Переходим к шкалам со средним риском
            remaining_medium_risk = [s for s in st.session_state.medium_risk_scales if s not in st.session_state.evaluated_scales]
...
def show_results():
    ...
    **Семейное положение**: {questionnaire.get('marital_status', 'Не указано')}
    ...
    if critical_issues:
        st.error(f"""
        **❌ Рекомендуется проведение дополнительного интервью с психологом**
        
        Выявлены критические факторы риска:
        {chr(10).join('• ' + issue for issue in critical_issues)}
        """)
        recommendation_color = "🔴"
        final_recommendation = "Рекомендуется проведение дополнительного интервью с психологом"
    elif warning_issues:
        st.warning(f"""
    ...
    if hasattr(st.session_state, 'recommendations') and st.session_state.recommendations:
        ...
        st.info("Рекомендации будут сформированы после завершения всех этапов тестирования.")
